...
// --------------------------------------------------------------
  ...
  async undoLastChange() {
    ...
    this.refreshDisplayedShapeLabel();
      ...
      if (snapshot.hasSpa) {
        if (!this.spa) {
          this.spa = createSpa(this.poolParams, this.scene, { tileSize: this.tileSize, shape: document.getElementById("spaShape")?.value || "square" });
    this.spa.userData.poolGroup = this.poolGroup || null;
          this.spa.userData.poolGroup = this.poolGroup || null;
        }
        if (snapshot.spa) {
          this.spa.userData.spaShape = snapshot.spa.spaShape || this.spa.userData.spaShape || "square";
          const spaShape = document.getElementById("spaShape");
          if (spaShape) spaShape.value = this.spa.userData.spaShape;
...
// PBR / Caustics integration should never hard-crash the app if a module fails
  ...
  setupSpaSystem() {
    ...
    const getAddSpaLabel = () => ((spaShape?.value || "square") === "circle" ? "Add Circle Spa" : "Add Square Spa");
    btn.textContent = getAddSpaLabel();

    this.setSpaSlidersEnabled(false);

    spaShape?.addEventListener("change", async (e) => {
      const shape = e.target.value || "square";
      if (!this.spa) {
        btn.textContent = getAddSpaLabel();
        return;
      }
      this.captureUndoState("Spa shape");
      this.spa.userData.spaShape = shape;
      if (shape === "circle") {
        const d = Math.min(this.spa.userData.spaLength || 2, this.spa.userData.spaWidth || 2);
        this.spa.userData.spaLength = d;
        this.spa.userData.spaWidth = d;
        const l = document.getElementById("spaLength");
        const w = document.getElementById("spaWidth");
        const lv = document.getElementById("spaLength-val");
        const wv = document.getElementById("spaWidth-val");
        if (l) l.value = d;
        if (w) w.value = d;
        if (lv) lv.textContent = d.toFixed(2) + " m";
        if (wv) wv.textContent = d.toFixed(2) + " m";
      }
      updateSpa(this.spa);
      await this.pbrManager.applyTilesToSpa(this.spa);
      if (this.poolGroup) {
  ...
  setupSpaSliders() {
    ["spaLength", "spaWidth", "spaTopHeight"].forEach((id) => {
      ...
      slider.addEventListener("input", async (e) => {
        ...
        if ((this.spa.userData?.spaShape || "square") === "circle" && (id === "spaLength" || id === "spaWidth")) {
          this.spa.userData.spaLength = val;
          this.spa.userData.spaWidth = val;
          const otherId = id === "spaLength" ? "spaWidth" : "spaLength";
          const otherSlider = document.getElementById(otherId);
          const otherOutput = document.getElementById(`${otherId}-val`);
          if (otherSlider) otherSlider.value = String(val);
          if (otherOutput) otherOutput.textContent = val.toFixed(2) + " m";
        }
