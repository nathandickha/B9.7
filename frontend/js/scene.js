...
function addDebugCylinder(group, radius, height, worldCenter, color, name, radialSegments = 48) {
  ...
  const edges = new THREE.LineSegments(
    new THREE.EdgesGeometry(new THREE.CylinderGeometry(radius, radius, height, radialSegments)),
    new THREE.LineBasicMaterial({ color, transparent: true, opacity: 0.9 })
  );
...
function updateSpaVoidDebug(scene, ground, poolGroup, spaGroup, holePts = null) {
  ...
  if ((spaGroup.userData?.spaShape || 'square') === 'circle') {
    const diameter = Math.max(1.0, Math.min(spaGroup.userData?.spaLength || 2, spaGroup.userData?.spaWidth || 2));
    const spaRadius = diameter * 0.5;
    const channelMargins = getSpaChannelMargins(spaGroup, poolGroup);
    const clipRadius = spaRadius + Math.max(channelMargins.minX || 0, channelMargins.maxX || 0, channelMargins.minY || 0, channelMargins.maxY || 0, 0) - SPA_GROUND_CLIP_MARGIN_REDUCTION + 0.02;
    if (ENABLE_SPA_DEBUG_VOIDS) {
      addDebugCylinder(group, Math.max(0.01, clipRadius), 0.03, new THREE.Vector3(center.x, center.y, groundZ + 0.015), 0xff00aa, 'SpaGroundClipDebugCircle');
    }
    if (ENABLE_ORANGE_INTERIOR_DEBUG) {
      const innerRadius = Math.max(0.05, spaRadius - SPA_WALL_THICKNESS - SPA_INTERIOR_CLEAR_WALL_INSET_XY - SPA_INTERIOR_CLEAR_EXTRA_INSET_XY);
      const minZ = spaGroup.position.z - spaGroup.userData.height * 0.5 + SPA_INTERIOR_CLEAR_EXTRA_INSET_Z;
      const maxZ = spaGroup.position.z + spaGroup.userData.height * 0.5 - SPA_INTERIOR_CLEAR_EXTRA_INSET_Z;
      addDebugCylinder(group, innerRadius, Math.max(0.01, maxZ - minZ), new THREE.Vector3(center.x, center.y, (minZ + maxZ) * 0.5), SPA_INTERIOR_CLEAR_DEBUG_COLOR, 'SpaInteriorClearDebugCircle');
    }
    if (ENABLE_SPA_DEBUG_VOIDS) {
      addDebugCylinder(group, spaRadius, Math.max(0.01, spaGroup.userData.height || 0.01), new THREE.Vector3(center.x, center.y, spaGroup.position.z), 0x00ff66, 'SpaBodyDebugCircle');
    }
    return;
  }
...
function addCircularChannelRing(group, innerRadius, outerRadius, height, center, material, zBottom, segments = 48, thetaStart = 0, thetaLength = Math.PI * 2) {
  if (!group || !(outerRadius > innerRadius) || !(height > 1e-4) || !(thetaLength > 1e-4)) return;
  const shape = new THREE.Shape();
  shape.absarc(0, 0, outerRadius, thetaStart, thetaStart + thetaLength, false);
  if (innerRadius > 1e-4) {
    const hole = new THREE.Path();
    hole.absarc(0, 0, innerRadius, thetaStart + thetaLength, thetaStart, true);
    shape.holes = [hole];
  }
  ...
  mesh.position.set(center.x, center.y, zBottom);
  mesh.castShadow = true;
  mesh.receiveShadow = true;
  mesh.userData.isSpaChannel = true;
  group.add(mesh);
...
function updateSpaChannelMeshes(ground, poolGroup, spaGroup) {
  ...
  if ((spaGroup.userData?.spaShape || 'square') === 'circle') {
    const diameter = Math.max(1.0, Math.min(spaGroup.userData?.spaLength || 2, spaGroup.userData?.spaWidth || 2));
    const spaRadius = diameter * 0.5;
    const channelRadius = spaRadius + Math.max(margins.minX || 0, margins.maxX || 0, margins.minY || 0, margins.maxY || 0, 0.01);

    let gapCenter = 0;
    let d = 0;
    if (spaGroup.userData?.snapSide === 'left') {
      gapCenter = 0.0;
      d = Math.max(-channelRadius, Math.min(channelRadius, (getPoolFootprintBoundsWorld(poolGroup)?.minX ?? center.x) - center.x));
    } else if (spaGroup.userData?.snapSide === 'right') {
      gapCenter = Math.PI;
      d = Math.max(-channelRadius, Math.min(channelRadius, center.x - (getPoolFootprintBoundsWorld(poolGroup)?.maxX ?? center.x)));
    } else if (spaGroup.userData?.snapSide === 'front') {
      gapCenter = Math.PI * 0.5;
      d = Math.max(-channelRadius, Math.min(channelRadius, (getPoolFootprintBoundsWorld(poolGroup)?.minY ?? center.y) - center.y));
    } else if (spaGroup.userData?.snapSide === 'back') {
      gapCenter = -Math.PI * 0.5;
      d = Math.max(-channelRadius, Math.min(channelRadius, center.y - (getPoolFootprintBoundsWorld(poolGroup)?.maxY ?? center.y)));
    }
    const alpha = Math.acos(Math.max(-1, Math.min(1, d / Math.max(0.001, channelRadius))));
    const thetaStart = gapCenter + alpha;
    const thetaLength = Math.max(0.01, Math.PI * 2 - alpha * 2);

    addCircularChannelRing(group, spaRadius, channelRadius, channelFloorThickness, center, mat, channelFloorTopZ - channelFloorThickness, 56, thetaStart, thetaLength);
    addCircularChannelRing(group, Math.max(spaRadius, channelRadius - 0.20), channelRadius, wallHeight, center, mat, channelFloorTopZ, 56, thetaStart, thetaLength);
    addCircularChannelRing(group, Math.max(spaRadius, channelRadius - 0.25), channelRadius, copingDepth, center, copingMat, copingUnderZ, 56, thetaStart, thetaLength);

    if (ground?.parent) ground.parent.add(group);
    ground.userData = ground.userData || {};
    ground.userData.spaChannelGroup = group;
    return;
  }
...
function ensureGroundSpaClipMaterial(ground) {
  ...
  mat.userData.spaClipUniforms = {
    ...
    spaCircleEnabled: { value: 0 },
    spaCircleCenter: { value: new THREE.Vector3() },
    spaCircleRadius: { value: 0.5 },
    spaCircleWallMode: { value: 0 },
    spaCircleWallValue: { value: 0.0 },
    ...
    shader.fragmentShader = shader.fragmentShader
      .replace(
        ...
        `#include <common>
         ...
         uniform int spaCircleEnabled;
         uniform vec3 spaCircleCenter;
         uniform float spaCircleRadius;
         uniform int spaCircleWallMode;
         uniform float spaCircleWallValue;
      ...
      .replace(
        ...
        `#include <clipping_planes_fragment>
         ...
         if (spaCircleEnabled == 1) {
           vec2 cd = vSpaClipWorldPos.xy - spaCircleCenter.xy;
           bool insideCircle = length(cd) <= spaCircleRadius;
           bool outsidePoolSide = false;
           if (spaCircleWallMode == 1) outsidePoolSide = vSpaClipWorldPos.x <= spaCircleWallValue;
           else if (spaCircleWallMode == 2) outsidePoolSide = vSpaClipWorldPos.x >= spaCircleWallValue;
           else if (spaCircleWallMode == 3) outsidePoolSide = vSpaClipWorldPos.y <= spaCircleWallValue;
           else if (spaCircleWallMode == 4) outsidePoolSide = vSpaClipWorldPos.y >= spaCircleWallValue;
           if (insideCircle && outsidePoolSide) discard;
         }
  ...
  mat.customProgramCacheKey = () => 'ground-spa-clip-v3';
...
function updateGroundMaterialSpaClip(ground, spaGroup = null, poolGroup = null) {
  ...
  if (!spaGroup) {
    ...
    uniforms.spaCircleEnabled.value = 0;
    uniforms.spaCircleWallMode.value = 0;
  ...
  if ((spaGroup.userData?.spaShape || 'square') === 'circle') {
    const diameter = Math.max(1.0, Math.min(spaGroup.userData?.spaLength || 2, spaGroup.userData?.spaWidth || 2));
    const radius = diameter * 0.5 + Math.max(margins.minX || 0, margins.maxX || 0, margins.minY || 0, margins.maxY || 0, 0) - SPA_GROUND_CLIP_MARGIN_REDUCTION + pad;
    const center = new THREE.Vector3();
    spaGroup.getWorldPosition(center);
    uniforms.spaClipEnabled.value = 0;
    uniforms.spaCircleEnabled.value = 1;
    uniforms.spaCircleCenter.value.copy(center);
    uniforms.spaCircleRadius.value = Math.max(0.01, radius);
    const bounds = getPoolFootprintBoundsWorld(poolGroup);
    const snapSide = spaGroup.userData?.snapSide || null;
    if (snapSide === 'left' && bounds) {
      uniforms.spaCircleWallMode.value = 1;
      uniforms.spaCircleWallValue.value = bounds.minX;
    } else if (snapSide === 'right' && bounds) {
      uniforms.spaCircleWallMode.value = 2;
      uniforms.spaCircleWallValue.value = bounds.maxX;
    } else if (snapSide === 'front' && bounds) {
      uniforms.spaCircleWallMode.value = 3;
      uniforms.spaCircleWallValue.value = bounds.minY;
    } else if (snapSide === 'back' && bounds) {
      uniforms.spaCircleWallMode.value = 4;
      uniforms.spaCircleWallValue.value = bounds.maxY;
    } else {
      uniforms.spaCircleWallMode.value = 0;
    }
