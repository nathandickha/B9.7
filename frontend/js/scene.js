...
function addCircularChannelRing(group, innerRadius, outerRadius, height, center, material, zBottom, segments = 48) {
  if (!group || !(outerRadius > innerRadius) || !(height > 1e-4)) return;
...
function updateSpaChannelMeshes(ground, poolGroup, spaGroup) {
  ...
  if ((spaGroup.userData?.spaShape || 'square') === 'circle') {
    ...
    addCircularChannelRing(group, spaRadius, channelRadius, channelFloorThickness, center, mat, channelFloorTopZ - channelFloorThickness, 56);
    addCircularChannelRing(group, Math.max(spaRadius, channelRadius - 0.20), channelRadius, wallHeight, center, mat, channelFloorTopZ, 56);
    addCircularChannelRing(group, Math.max(spaRadius, channelRadius - 0.25), channelRadius, copingDepth, center, copingMat, copingUnderZ, 56);
