...
function updateSpaChannelMeshes(ground, poolGroup, spaGroup) {
  ...
  if ((spaGroup.userData?.spaShape || 'square') === 'circle') {
    ...
    let gapCenter = 0;
    if (spaGroup.userData?.snapSide === 'left') gapCenter = 0.0;
    else if (spaGroup.userData?.snapSide === 'right') gapCenter = Math.PI;
    else if (spaGroup.userData?.snapSide === 'front') gapCenter = Math.PI * 0.5;
    else if (spaGroup.userData?.snapSide === 'back') gapCenter = -Math.PI * 0.5;
    const hiddenArc = Math.PI * 0.70;
    const thetaStart = gapCenter + hiddenArc * 0.5;
    const thetaLength = Math.PI * 2 - hiddenArc;
    ...
    const bridgeWidth = (() => {
      if (spaGroup.userData?.snapSide === 'left') return margins.maxX || 0;
      if (spaGroup.userData?.snapSide === 'right') return margins.minX || 0;
      if (spaGroup.userData?.snapSide === 'front') return margins.maxY || 0;
      if (spaGroup.userData?.snapSide === 'back') return margins.minY || 0;
      return 0;
    })();

    if (bridgeWidth > 0.001) {
      const stripThickness = channelFloorThickness;
      if (spaGroup.userData?.snapSide === 'left') {
        addChannelBox(group, bridgeWidth, diameter, stripThickness, spaRadius + bridgeWidth * 0.5, 0, center, axisX, axisY, quat, mat, floorZCenter);
      } else if (spaGroup.userData?.snapSide === 'right') {
        addChannelBox(group, bridgeWidth, diameter, stripThickness, -(spaRadius + bridgeWidth * 0.5), 0, center, axisX, axisY, quat, mat, floorZCenter);
      } else if (spaGroup.userData?.snapSide === 'front') {
        addChannelBox(group, diameter, bridgeWidth, stripThickness, 0, spaRadius + bridgeWidth * 0.5, center, axisX, axisY, quat, mat, floorZCenter);
      ...
      .replace(
        ...
        `#include <clipping_planes_fragment>
         ...
         if (spaCircleEnabled == 1) {
           ...
           if (length(cd) <= spaCircleRadius) discard;
         }
