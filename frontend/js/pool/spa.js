...
function updateCircularSpaSeats(spa) {
  ...
  const w = spa.userData.spaWidth;
  const h = spa.userData.height;
  const radius = Math.max(0.35, Math.min(l, w) * 0.5 - SPA_WALL_THICKNESS);
  const seatOuter = Math.max(0.15, radius - 0.05);
  ...
  const geo = new THREE.CylinderGeometry(seatOuter, seatOuter, SPA_SEAT_THICKNESS, 48, 1, false, 0, Math.PI * 2);
  generateMeterUVsForBoxGeometry(new THREE.BoxGeometry(1,1,1), spa.userData.tileSize || 0.3);
  seat.geometry.dispose();
  seat.geometry = geo;
  seat.rotation.set(Math.PI * 0.5, 0, 0);
  seat.position.set(0, 0, seatCenterLocal);
  seat.scale.set(1, 1, 1);
  seat.visible = true;
...
function updateCircularSpaWalls(spa) {
  ...
  const l = spa.userData.spaLength;
  const w = spa.userData.spaWidth;
  const diameter = Math.max(1.0, Math.min(l, w));
  ...
  const seats = spa.userData.seats || {};
  [seats.front, seats.back, seats.left, seats.right].forEach((s) => { if (s) s.visible = false; });
