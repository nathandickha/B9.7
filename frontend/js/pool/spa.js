...
export function setSpaTopOffset(val) {
  ...
  if (selectedSpa) {
    ...
    if ((selectedSpa.userData?.spaShape || "square") !== "circle") updateSpaSeats(selectedSpa);
...
function updateSpaWalls(spa) {
  if ((spa.userData?.spaShape || "square") === "circle") {
    updateCircularSpaWalls(spa);
    return;
  }
  if (spa.userData.circleWall) spa.userData.circleWall.visible = false;
  Object.values(spa.userData.walls || {}).forEach((wall) => { if (wall) wall.visible = true; });
  [spa.userData.seats?.front, spa.userData.seats?.back, spa.userData.seats?.left, spa.userData.seats?.right].forEach((s) => { if (s) s.visible = true; });
  const water = spa.userData.waterMesh;
  const walls = spa.userData.walls;
  const poolParams = spa.userData.poolParams;
  if (spa.userData.circleWall) spa.userData.circleWall.visible = false;
  Object.values(walls || {}).forEach((wall) => { if (wall) wall.visible = true; });
  Object.values(spa.userData.seats || {}).forEach((seat) => { if (seat) seat.visible = true; });
  ...
  // --- Water ---
  // Restore the original volumetric spa water footprint so the vertical water
  // walls form correctly, but keep the pool-matching shader/prepass inputs.
  ...
  const waterCenterLocal = waterBottomWorld + waterHeight / 2 - spa.position.z;
  ...
  if (water?.userData?.waterUniforms) {
    ...
    if (u.thicknessShallow) u.thicknessShallow.value = 0.30;
    if (u.alphaShallow) u.alphaShallow.value = 0.18;
    if (u.alphaDeep) u.alphaDeep.value = 0.88;
  }
...
export function snapToPool(spa) {
  ...
  const isCircle = (spa.userData?.spaShape || "square") === "circle";
  const circleDiameter = Math.max(0.01, Math.min(l, w));
  const circleRadius = circleDiameter * 0.5;
  ...
  if (close === "left") {
    const outerPos = minX - (isCircle ? circleRadius : l * 0.5) - SPA_OUTER_WALL_ALIGN_OFFSET;
    const centerPos = minX;
    const innerPos = minX + (isCircle ? circleRadius : l * 0.5) + dynamicSnap + wallNudge;
  ...
  if (close === "right") {
    const outerPos = maxX + (isCircle ? circleRadius : l * 0.5) + SPA_OUTER_WALL_ALIGN_OFFSET;
    const centerPos = maxX;
    const innerPos = maxX - (isCircle ? circleRadius : l * 0.5) - dynamicSnap - wallNudge;
    const snapToOuter = Math.abs(x - outerPos) <= SPA_WALL_CENTER_SNAP_THRESHOLD;
    const snapToCenter = Math.abs(x - centerPos) <= SPA_WALL_CENTER_SNAP_THRESHOLD;
  ...
  if (close === "front") {
    const outerPos = minY - (isCircle ? circleRadius : w * 0.5) - SPA_OUTER_WALL_ALIGN_OFFSET;
    const centerPos = minY;
    const innerPos = minY + (isCircle ? circleRadius : w * 0.5) + dynamicSnap + wallNudge;
  ...
  if (close === "back") {
    const outerPos = maxY + (isCircle ? circleRadius : w * 0.5) + SPA_OUTER_WALL_ALIGN_OFFSET;
    const centerPos = maxY;
    const innerPos = maxY - (isCircle ? circleRadius : w * 0.5) - dynamicSnap - wallNudge;
    const snapToOuter = Math.abs(y - outerPos) <= SPA_WALL_CENTER_SNAP_THRESHOLD;
    const snapToCenter = Math.abs(y - centerPos) <= SPA_WALL_CENTER_SNAP_THRESHOLD;
...
function createCircleShape(radius, segments = 48) {
  const shape = new THREE.Shape();
  shape.absarc(0, 0, radius, 0, Math.PI * 2, false);
  return shape;
}

function createRingExtrudeGeometry(outerRadius, innerRadius, height, segments = 48) {
  const shape = createCircleShape(Math.max(0.01, outerRadius), segments);
  const hole = new THREE.Path();
  hole.absarc(0, 0, Math.max(0.005, innerRadius), 0, Math.PI * 2, true);
  shape.holes = [hole];
  const geo = new THREE.ExtrudeGeometry(shape, {
...
function updateCircularSpaSeats(spa) {
  const seats = spa.userData.seats || {};
  const seat = seats.front;
  if (!seat) return;

  const l = spa.userData.spaLength;
  const w = spa.userData.spaWidth;
  const h = spa.userData.height;
  const innerWallRadius = Math.max(0.35, Math.min(l, w) * 0.5 - SPA_WALL_THICKNESS);
  const seatOuter = Math.max(0.15, innerWallRadius - 0.05);
  const seatInner = Math.max(0.05, seatOuter - SPA_SEAT_DEPTH);

  const spaTop = spa.position.z + h / 2;
  const seatTopAbs = spaTop - SPA_SEAT_TOP_OFFSET;
  const seatCenterAbs = seatTopAbs - SPA_SEAT_THICKNESS * 0.5;
  const seatCenterLocal = seatCenterAbs - spa.position.z;

  const geo = createRingExtrudeGeometry(seatOuter, seatInner, SPA_SEAT_THICKNESS, 48);
  seat.geometry.dispose();
  seat.geometry = geo;
  seat.rotation.set(0, 0, 0);
  seat.position.set(0, 0, seatCenterLocal - SPA_SEAT_THICKNESS * 0.5);
  seat.scale.set(1, 1, 1);
  seat.visible = true;

  [seats.back, seats.left, seats.right].forEach((s) => { if (s) s.visible = false; });
}

function updateCircularSpaWalls(spa) {
  const water = spa.userData.waterMesh;
  const poolParams = spa.userData.poolParams;
  const wallMesh = spa.userData.circleWall;
  const floor = spa.userData.floor;
  const walls = spa.userData.walls || {};
  ...
  const diameter = Math.max(1.0, Math.min(spa.userData.spaLength || 2, spa.userData.spaWidth || 2));
  spa.userData.spaLength = diameter;
  spa.userData.spaWidth = diameter;
  const outerRadius = diameter * 0.5;
  const innerRadius = Math.max(0.2, outerRadius - SPA_WALL_THICKNESS);

  Object.values(walls).forEach((wall) => { if (wall) wall.visible = false; });
  if (wallMesh) {
    const geo = createRingExtrudeGeometry(outerRadius, innerRadius, h, 48);
    wallMesh.geometry.dispose();
    wallMesh.geometry = geo;
    wallMesh.position.set(0, 0, -h * 0.5);
    wallMesh.rotation.set(0, 0, 0);
    wallMesh.visible = true;
  }

  updateCircularSpaSeats(spa);

  const waterBottomWorld = SPA_WATER_BOTTOM_WORLD;
  const poolWaterTop = SPA_TOP_OFFSET + SPA_WATER_TOP_FINE_ADJUST;
  const waterHeight = Math.max(0.01, poolWaterTop - waterBottomWorld);
  const waterRadius = innerRadius + 0.5 * WATER_OVERFLOW;
  const waterGeo = new THREE.CylinderGeometry(waterRadius, waterRadius, waterHeight, 48);
  ...
  if (floor) {
    const floorHeight = 0.2;
    const geo = new THREE.CylinderGeometry(innerRadius, innerRadius, floorHeight, 48);
    floor.geometry.dispose();
    floor.geometry = geo;
...
export function createSpa(poolParams, scene, options = {}) {
  ...
  spa.userData.spaShape = options.shape || "square";
  ...
  const circleWall = new THREE.Mesh(new THREE.BoxGeometry(1,1,1), wallMat.clone());
  circleWall.castShadow = true;
  circleWall.receiveShadow = true;
  circleWall.userData.isSpaWall = true;
  circleWall.visible = false;
  spa.add(circleWall);
  spa.userData.circleWall = circleWall;
...
export function updateSpa(spa) {
  ...
  snapToPool(spa);
  updateSpaWalls(spa);
  if ((spa.userData?.spaShape || "square") !== "circle") updateSpaSeats(spa);
  snapToPool(spa);
}
